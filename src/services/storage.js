...
function applyExtensions(_) {
  ...
  // @TODO localStorage changes update stores?

  // storage.update('my_array', my_array => [].concat(my_array, [1, 2, 3]));
  ...
  _.fn('persistStore', function(key, store) {
    store.stopPersisting = store.subscribe(val => this.set(key, val == null ? null : val));
    return store;
  });
  ...
  _.get = function(area, key, kid) {
    ...
    if (s == null) {
      ...
      if (parts) {
        ...
        kid = kid ? path + '.' + kid : path;
        return _.get(area, key, kid);
      }
    } else if (kid) {
      return _.stringify(getDeep(_.parse(s), kid));
    }
    return s;
  };

  function getDeep(obj, path, alt) {
  ...
  _.on = function(key, fn) {
    ...
    var s = this,
      listener = function(e) {
        ...
        if (
          // match key if listener
          // has one
          (k && (k === key ||
            // match catch-all, except
            // internal test
            (!key && k !== '_-bad-_'))) &&
